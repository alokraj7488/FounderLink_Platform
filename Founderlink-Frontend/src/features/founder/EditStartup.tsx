...
interface EditStartupParams { id: string; [key: string]: string | undefined; }
...
const EditStartup: React.FC = () => {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<StartupFormData>({ resolver: yupResolver(startupSchema) });
  const navigate = useNavigate();
  const startupId = Number(id);
  const watched = watch();

  useEffect(() => {
    if (!id || Number.isNaN(startupId)) { toast.error('Invalid startup'); return; }
    getStartupById(startupId).then((res) => reset(res.data)).catch(() => toast.error('Failed to load startup'));
  }, [id, reset, startupId]);
    ...
    <Layout>
      <div style={{ maxWidth: 1040, margin: '0 auto' }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: "'Syne', system-ui, sans-serif", fontSize: 28, fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>Edit startup</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: 14, marginTop: 4 }}>Update your startup details. Changes will be reviewed by admin.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: 20, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, boxShadow: 'var(--shadow-card)' }}>
            <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div>
                ...
                <textarea rows={3} className="input-field" style={{ height: 'auto', resize: 'vertical' }} placeholder="Describe your startup…" {...register('description', { required: 'Required' })} />
          ...
          <div style={{ position: 'sticky', top: 20 }}>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, boxShadow: 'var(--shadow-card)' }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 12, minHeight: 36 }}>
